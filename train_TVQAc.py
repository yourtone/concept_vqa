'''
train_TVQAc.py from train_TVQA.py
2019/05/18 22:36 @yourtone
e.g.
python train_TVQAc.py
...
def predict_train(train_loader, model):
    ...
    for sample in bar(train_loader):
        ...
        score = torch.bmm(sample_var[-1], fuse_emb.unsqueeze(2)).squeeze() # (64, 50)
...
def format_result(que_ids, scores, qid_ocr):
    ...
    for i, que_id in enumerate(que_ids):
        ...
        _, ans_id = torch.max(scores.data[i][:len(itoa)], dim=0)
